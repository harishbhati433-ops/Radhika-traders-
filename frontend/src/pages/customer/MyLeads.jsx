...
const TONE = { pending: "bg-amber-50 text-amber-700", approved: "bg-emerald-50 text-emerald-700", rejected: "bg-rose-50 text-rose-700", duplicate: "bg-violet-50 text-violet-700", account_opened: "bg-sky-50 text-sky-700", trade_done: "bg-indigo-50 text-indigo-700" };
const A_LABEL = { pending: "Not Started", account_opened: "Account Open", trade_done: "Trade Done", rejected: "Account Rejected" };
  ...
  return (
    <DashboardLayout nav={customerNav} title="My Leads">
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {[["Total", leads.length, "bg-white text-slate-800"], ["Pending", count("status", "pending"), "bg-amber-50 text-amber-700"], ["Approved", count("status", "approved"), "bg-emerald-50 text-emerald-700"], ["Duplicate", count("status", "duplicate"), "bg-violet-50 text-violet-700"], ["Account Open", count("account_status", "account_opened"), "bg-sky-50 text-sky-700"], ["Trade Done", count("account_status", "trade_done"), "bg-indigo-50 text-indigo-700"]].map(([l, v, cls]) => (
      ...
      <div className="mb-4 grid gap-2 rounded-2xl border border-slate-200 bg-white p-3 sm:grid-cols-2 lg:grid-cols-4" data-testid="my-leads-filters">
        ...
        <select data-testid="my-leads-filter-status" value={status} onChange={(e) => setStatus(e.target.value)} className={sel}><option value="">All statuses</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option><option value="duplicate">Duplicate</option><option value="account_opened">Account Open</option><option value="trade_done">Trade Done</option></select>
      </div>
        ...
        {shown.map((l) => (
          <div key={l.id} data-testid={`my-lead-${l.id}`} className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-1.5"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold capitalize ${TONE[l.status]}`} data-testid={`my-lead-status-${l.id}`}>{l.status}</span><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${TONE[l.account_status]}`}>{A_LABEL[l.account_status] || l.account_status}</span></div>
            </div>
            {l.status === "duplicate" && (
              <div className="mt-2 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs text-violet-900" data-testid={`my-lead-duplicate-${l.id}`}>
                <b>Duplicate lead</b> — this person already submitted in this campaign (original: <span className="font-mono">{l.duplicate_of}</span>). {l.duplicate_reason}. Duplicate leads are not eligible for payout.
              </div>
            )}
            {l.details?.length > 0 && (
