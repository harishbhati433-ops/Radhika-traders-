...
export function ShutdownGate({ children }) {
  ...
  useEffect(() => {
    ...
    const onShutdown = (e) => { setState({ active: true, ...e.detail }); if (!window.location.pathname.startsWith("/admin")) { localStorage.removeItem("rt_token"); window.dispatchEvent(new Event("rt:logout")); } };
